...
public class BTreeFile implements DbFile {
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		while (iter.hasNext()) {
			...
			rightPage.insertEntry(ent);
		}

		//If the page being split is the root page, you will need to create a new internal node to become the new root page,
		// and update the BTreeRootPtrPage.
		...
		BTreeInternalPage parentPage = (BTreeInternalPage) getParentWithEmptySlots(tid,dirtypages,page.getParentId(),field);

		// rightPage is a new page, all its children were moved.
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful
